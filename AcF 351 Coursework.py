...
# just for informational pruposes - no relationships could be identified at later analysis stages but still left it here
Analysis['delta_polarity_LM'] = Analysis.groupby(['permno'])['polarity_LM'].pct_change()
Analysis['delta_polarity_B'] = Analysis.groupby(['permno'])['polarity_B'].pct_change()


# In[576]:


# add one more column with year data to get statistics and graphs per year
Analysis['year'] = Analysis['Filing_date'].dt.year
...
# because not useful as it turned out later
Analysis = Analysis.dropna(subset = ['delta_polarity_LM','delta_polarity_B'])
...
# ### More explatory data analysis

# In[581]:


# useful short-cut to get an overview over the whole dataset
Analysis.profile_report()


# In[8]:


# filings per day over the whole period
dataByYear = Analysis.groupby('Filing_date')['permno'].nunique()    
print(dataByYear)

plt.title(r'Company filings over the period')
...
dataByYear = Analysis.groupby('year')['permno'].nunique()    
print(dataByYear)

plt.title(r'Company filings per year')
plt.bar(list(dataByYear.index), list(dataByYear),width = 0.75)
plt.show()
...
# scatterplot textual analysis LM scores and 1 year returns 

Scatter_LM_returns_1year = Analysis.plot.scatter(x='polarity_LM',
                                                 y='n_to_1year_ret',
                                                 c='DarkBlue')
...
# Prepare Data
analysis_by_industry = Analysis.groupby('2017 NAICS Title').size()

# Make the plot with pandas
analysis_by_industry.plot(kind='pie', subplots=True, figsize=(8, 8))# dpi= 80)
plt.title('Industry classifications')
plt.ylabel("")
plt.show()

# takeaways: commercial banking and pharmaceutical make the biggest chunk of the pie apart from not availble 


# ## Returns and textual analysis focus

# In[1046]:


df = Analysis[['polarity_LM','polarity_B','delta_polarity_LM','delta_polarity_B','n-1_to_n_ret','n_to_n+1_ret','n_to_n+5_ret','n_to_1month_ret','n_to_1year_ret','NegBpercentage','NegLMpercentage','Overall_sentiment_B','Overall_sentiment_LM','year']]
...
# Decorations
plt.title('Correlogram of returns and textual analysis', fontsize=22)
...
df.quantile([0.2,0.4,0.6,0.8,1])


# In[1212]:


df = df.sort_values(by = ['polarity_LM'])
...
# quintiles
first_quantile = df.iloc[:9601]
second_quantile = df.iloc[9601:19202]
third_quantile = df.iloc[19202:28803]
forth_quantile = df.iloc[28803:38404]
fifth_quantile = df.iloc[38404:]


# In[1214]:


print(np.nanmedian(first_quantile['n-1_to_n_ret']))
print(np.nanmedian(second_quantile['n-1_to_n_ret']))
print(np.nanmedian(third_quantile['n-1_to_n_ret']))
print(np.nanmedian(forth_quantile['n-1_to_n_ret']))
print(np.nanmedian(fifth_quantile['n-1_to_n_ret']))


# In[1215]:


print(np.nanmedian(first_quantile['n_to_n+1_ret']))
print(np.nanmedian(second_quantile['n_to_n+1_ret']))
print(np.nanmedian(third_quantile['n_to_n+1_ret']))
print(np.nanmedian(forth_quantile['n_to_n+1_ret']))
print(np.nanmedian(fifth_quantile['n_to_n+1_ret']))


# In[1216]:


print(np.nanmedian(first_quantile['n_to_n+5_ret']))
print(np.nanmedian(second_quantile['n_to_n+5_ret']))
print(np.nanmedian(third_quantile['n_to_n+5_ret']))
print(np.nanmedian(forth_quantile['n_to_n+5_ret']))
print(np.nanmedian(fifth_quantile['n_to_n+5_ret']))


# In[1217]:


print(np.nanmedian(first_quantile['n_to_1month_ret']))
print(np.nanmedian(second_quantile['n_to_1month_ret']))
print(np.nanmedian(third_quantile['n_to_1month_ret']))
print(np.nanmedian(forth_quantile['n_to_1month_ret']))
print(np.nanmedian(fifth_quantile['n_to_1month_ret']))


# In[1218]:


print(np.nanmedian(first_quantile['n_to_1year_ret']))
print(np.nanmedian(second_quantile['n_to_1year_ret']))
print(np.nanmedian(third_quantile['n_to_1year_ret']))
print(np.nanmedian(forth_quantile['n_to_1year_ret']))
print(np.nanmedian(fifth_quantile['n_to_1year_ret']))


# # Return analysis on industry level

# In[980]:


df = Analysis[['2017 NAICS Title','polarity_LM','n_to_1year_ret','NegLMpercentage','year']]


# In[981]:


# prepare for dropping rows
df = df.set_index('2017 NAICS Title')
# dropping rows with 'no information available' as not relevant for industry analysis
df = df.drop('no information available', axis = 0)  # dropping  around 500 observations
# drop rows not containing an industry classification and 1 year returns
df = df.reset_index().dropna() # dropoing around 9000 observations


# In[984]:


df = df.groupby(['2017 NAICS Title']).agg({'n_to_1year_ret' : ['mean','median'],'2017 NAICS Title' : (np.count_nonzero)})


# In[986]:


# drop industry if less than 25 observations (25 is arbitry and is purely based on the fact to normalize outliers)
df = df[df[('2017 NAICS Title', 'count_nonzero')] > 25]


# In[992]:


# sort by median 1 year return
df = df.sort_values(by =[('n_to_1year_ret', 'median')])
...
df = df.groupby(['2017 NAICS Title']).agg({'polarity_LM' : ['mean','median'],'2017 NAICS Title' : (np.count_nonzero)})
...
# sort by median 1 year return
df = df.sort_values(by =[('polarity_LM', 'median')])


# In[1032]:


# worst performers by polarity scores
df.head()
...
# best performers by polarity scores
df.tail()


# # Sentiment analysis (Polarity LM) year by year 
...
df = df.groupby(['year']).agg({'polarity_LM' : ['mean','median'],'year' : (np.count_nonzero)})


# In[1004]:


#df = df[('polarity_LM', 'median')]


# In[1025]:


polarity_by_year = sns.boxplot(y='polarity_LM', x='year', 
                 data=df, 
                 width=0.5,
                 palette="colorblind")
...
negativeperc_by_year = sns.boxplot(y='NegLMpercentage', x='year', 
                 data=df, 
                 width=0.5,
                 palette="colorblind")


# # ESG analysis based on quantiles

# In[964]:


df = Analysis[['2017 NAICS Title','ESGpercentage','n_to_1year_ret','CIK','year']]
df = df.dropna()
...
df = df.sort_values(by = ['ESGpercentage'])
...
# first being the worst textual analysis scores and fifth being the best scores
first_quantile = df.iloc[:7688]
second_quantile = df.iloc[7688:15376]
third_quantile = df.iloc[15376:23064]
forth_quantile = df.iloc[23064:30752]
fifth_quantile = df.iloc[30752:]


# In[972]:


print(np.mean(first_quantile['n_to_1year_ret']))
print(np.mean(second_quantile['n_to_1year_ret']))
print(np.mean(third_quantile['n_to_1year_ret']))
print(np.mean(forth_quantile['n_to_1year_ret']))
print(np.mean(fifth_quantile['n_to_1year_ret']))


# In[973]:


print(np.median(first_quantile['n_to_1year_ret']))
print(np.median(second_quantile['n_to_1year_ret']))
print(np.median(third_quantile['n_to_1year_ret']))
print(np.median(forth_quantile['n_to_1year_ret']))
print(np.median(fifth_quantile['n_to_1year_ret']))


# # ESG analysis based on years
...
df = df.groupby(['year']).agg({'n_to_1year_ret' : ['mean','median'],'year' : (np.count_nonzero), 'ESGpercentage' : ['mean','median']})
...
# # Are industry returns independent of ESG scores?

# In[837]:


df = Analysis[['2017 NAICS Title','polarity_LM','ESGpercentage','n_to_1year_ret','NegLMpercentage','year']]
...
gf = df.groupby(['2017 NAICS Title']).agg({'n_to_1year_ret' : ['mean','median'],'2017 NAICS Title' : (np.count_nonzero), 'ESGpercentage' : ['mean','median']})


# In[814]:


# drop industry if less than 15 observations (15 is arbitry and is purely based on the fact to normalize outliers)
gf = gf[gf[('2017 NAICS Title', 'count_nonzero')] > 25]


# In[821]:


gf['ESGrank'] = gf[('ESGpercentage', 'median')].rank()
gf['Retrank'] = gf[('n_to_1year_ret', 'median')].rank()
...
# H0: come from the same sample 
# HA: come from samples that differ in location


# In[806]:


stat, p = wilcoxon(gf['Retrank'], gf['ESGrank'])
...
alpha = 0.05
if p > alpha:
	print('Same distribution (fail to reject H0) i.e. no benefit of investing in high % ESG industries')
else:
	print('Different distribution (reject H0)')


# # Is firm share performance independent of ESG scores? (every year)

# In[842]:


ef = Analysis[['2017 NAICS Title','ESGpercentage','n_to_1year_ret']]
ef = ef.dropna()
...
ef['ESGrank'] = ef['ESGpercentage'].rank()
ef['Retrank'] = ef['n_to_1year_ret'].rank()
...
stat, p = wilcoxon(ef['Retrank'], ef['ESGrank'])
...
alpha = 0.05
if p > alpha:
	print('Same distribution (fail to reject H0) i.e. no benefit of investing in high % ESG stocks')
else:
	print('Different distribution (reject H0)')


# # Is firm share performance independent of ESG scores? (1 firm 1 ESG rating)

# In[860]:


df = Analysis[['2017 NAICS Title','ESGpercentage','n_to_1year_ret','CIK']]
df = df.dropna()
...
df = df.groupby(['CIK']).agg({'n_to_1year_ret' : ['mean','median'],'CIK' : (np.count_nonzero), 'ESGpercentage' : ['mean','median']})


# In[863]:


df['ESGrank'] = df[('ESGpercentage', 'median')].rank()
df['Retrank'] = df[('n_to_1year_ret', 'median')].rank()


# In[864]:


stat, p = wilcoxon(ff['Retrank'], ff['ESGrank'])
...
# # ESG analysis on industry level post 2016 (given the recent ESG trend)
...
df = df.groupby(['2017 NAICS Title']).agg({'n_to_1year_ret' : ['mean','median'],'2017 NAICS Title' : (np.count_nonzero), 'ESGpercentage' : ['mean','median']})
...
stat, p = wilcoxon(ff2['Retrank'], ff2['ESGrank'])
...
# # Only for illustrative purposes for the written report

# ## Difference between cleaned and uncleaned financial statements

# ### Tesla 2017 Financial statement
